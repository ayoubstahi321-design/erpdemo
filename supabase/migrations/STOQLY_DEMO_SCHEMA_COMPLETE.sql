...
-- Un solo archivo — borra y recrea todo desde cero
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  warehouse_id  UUID,        -- Almacén asignado (NULL = acceso a todos)
  company_id    TEXT,        -- Empresa asignada (NULL = Admin ve todas)
...
-- ============================================================
-- 6. STOCK LEVELS (Stock por Almacén)
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.sales (
  ...
  document_type         TEXT DEFAULT 'INVOICE' CHECK (document_type IN ('TICKET','INVOICE','DELIVERY_NOTE','QUOTE')),
  company_id            TEXT REFERENCES public.companies(id) ON DELETE SET NULL,
  -- Financial
...
CREATE TABLE IF NOT EXISTS public.payments (
  ...
  bank_name       TEXT,
  due_date        DATE,
  payment_status  TEXT DEFAULT 'Pending' CHECK (payment_status IN ('Pending','Cashed','Bounced')),
...
-- ============================================================
-- 15. PURCHASE ORDERS (Órdenes de Compra)
-- ============================================================
...
-- ============================================================
-- 16. DOCUMENT COUNTERS (Numeración automática)
-- ============================================================
...
-- ============================================================
-- 17. APP SETTINGS (Configuración global)
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_sale_items_prod   ON public.sale_items(product_id);
CREATE INDEX IF NOT EXISTS idx_payments_sale     ON public.payments(sale_id);
CREATE INDEX IF NOT EXISTS idx_transfers_from    ON public.transfers(from_warehouse_id);
CREATE INDEX IF NOT EXISTS idx_transfers_to      ON public.transfers(to_warehouse_id);
...
-- Company
INSERT INTO public.companies (id, name, full_name, address, city, country, phone, email, website,
  ice, rc, if_number, default_tax_rate, currency_symbol, invoice_prefix)
VALUES (
  'demo-company-001',
  'Stoqly Demo',
  'DEMO COMPANY SARL',
  '45 Bd. Hassan II, Zone Industrielle',
  'Casablanca',
  'Maroc',
  '+212 522 00 11 22',
  'demo@stoqly.com',
...
VALUES (
  ...
  'Global Lubricants Supply',
  'Ahmed Benali',
  'supply@globallub.com',
  '+212 600 11 22 33',
  'Casablanca',
  'Maroc'
) ON CONFLICT (id) DO NOTHING;

-- Products (10 generic lubricant products)
INSERT INTO public.products (id, sku, name, category, viscosity, pack_size, unit, units_per_box, price, cost, min_stock, supplier_id) VALUES
